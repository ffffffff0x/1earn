...
存在一种保护措施来应对这种攻击。这种保护的原理是将身份验证层与正在使用的协议绑定，即使存在的 TLS 层也将其绑定（例如 LDAPS 或 HTTPS）。通常的想法是，在最后一条 NTLMAUTHENTICATE 消息中，一条信息被放置在此处，并且攻击者无法对其进行修改。该信息指示所需的服务，并可能指示包含目标服务器证书的哈希值的另一信息。
